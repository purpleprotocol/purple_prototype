...
pub fn eligibility_requirement(node_count: u16) -> u16 { ((node_count as f32 + 1.0) / 2.0).trunc() as u16 }

/// The number of other events from distinct nodes
/// that a node's last event must be followed by in 
/// order to be eligible to send a `Heartbeat` event.
pub fn heartbeat_requirement(node_count: u16) -> u16 { ((node_count as f32 + 1.0) / 3.0).trunc() as u16 }

/// The number of other events from distinct nodes
/// that a voting event must be followed by in order
/// to propose for a `CanidateSet`.
pub fn proposal_requirement(node_count: u16) -> u16 { ((node_count as f32 + 1.0) / 2.0).trunc() as u16 }

/// The number of required proposals for a `CandidateSet`
/// in order to be considered valid for inclusion into
/// the total order.
pub fn required_proposal(node_count: u16) -> u16 { node_count + 1 }
...
pub fn leave_requirement(node_count: u16) -> u16 { (2.0f32.powf(8.0f32) * (node_count as f32).ln()).trunc() as u16 }
...
pub fn leave_limit(node_count: u16) -> u16 { (2.0f32.powf(10.0f32) * (node_count as f32).ln()).trunc() as u16 }